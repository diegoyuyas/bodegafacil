...
import {
  construirPlantillaImportarProveedores,
  filaEstaVacia,
  marcarDuplicadosProveedor,
  parsearFilaImportacionProveedor,
  type FilaImportadaProveedor,
} from '@/core/importacion-proveedores';
...
export default function PaginaImportarProveedores() {
  const { contenedor, cargando, error } = usarContenedor();
  const [estadoPlan, setEstadoPlan] = useState<EstadoPlan | null>(null);
  const inputArchivoRef = useRef<HTMLInputElement>(null);

  const [filas, setFilas] = useState<FilaImportadaProveedor[]>([]);
  ...
  async function descargarPlantilla() {
    const libro = generarLibroExcel([construirPlantillaImportarProveedores()]);
    await descargarExcel('vende-facil-plantilla-proveedores.xlsx', libro);
  }
  ...
  async function manejarArchivoSeleccionado(archivos: FileList | null) {
    ...
    try {
      ...
      const procesadas: FilaImportadaProveedor[] = [];
      filasDatos.forEach((valores, indice) => {
        if (filaEstaVacia(valores)) return;
        procesadas.push(parsearFilaImportacionProveedor(valores, indice + 2));
      });

      const nombresExistentes = new Set(
        contenedor.proveedores.listarTodos().map((p) => p.nombre.trim().toLowerCase()),
      );
      const conDuplicadosMarcados = marcarDuplicadosProveedor(procesadas, nombresExistentes);
  ...
  async function importar() {
    ...
    for (const fila of filas) {
      ...
      try {
        contenedor.proveedores.crear(fila.datos.nombre, fila.datos.ruc, fila.datos.telefono);
        creados += 1;
      } catch {
        conError += 1;
      }
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-app flex-col px-5 pb-24 pt-6">
      <header className="flex items-center gap-3">
        ...
        <h1 className="text-lg font-extrabold text-bodega-oscuro">Importar Proveedores</h1>
      </header>
      ...
      <p className="mt-4 text-sm text-tinta/70">
        Carga varios proveedores de una sola vez: descarga la plantilla, complétala y súbela de vuelta.
      </p>

      {estadoPlan && !esPremium && (
        <section className="mt-6 rounded-xl border border-linea p-5 text-center">
          <p className="text-sm font-semibold text-tinta">Importar Proveedores es una función Premium</p>
        ...
        <>
          <section className="mt-6">
            ...
            <p className="mt-1 text-xs text-tinta/50">
              Columnas: nombre_proveedor, ruc, telefono. Solo el nombre es obligatorio.
            </p>
            <button
          ...
          {filas.length > 0 && (
            <section className="mt-8">
              ...
              <p className="mt-1 text-xs text-tinta/60">
                {filasValidas.length} proveedor(es) listos ·{' '}
                  ...
                  <li key={fila.numeroFila} className="p-3 text-xs">
                    {fila.datos ? (
                      <p className="text-tinta">
                        ...
                        {fila.datos.ruc ? ` · RUC ${fila.datos.ruc}` : ''}
              ...
              >
                {importando ? 'Importando…' : `Importar ${filasValidas.length} proveedor(es)`}
              </button>
            </section>
          )}
          ...
          {resultado && (
            <section className="mt-8 rounded-xl border border-linea p-4 text-sm">
              ...
              <p className="mt-1 text-tinta/70">{resultado.creados} proveedor(es) creado(s).</p>
